			       ...
			       const u8 *data, size_t len, int rx_freq)
{
	...
	parse_result = wfd_parse_ies(data + 1, len - 1, &wfd_msg);
	/* parse_result: 
	...
	if (p2p->wfd->cfg->enabled ) {
		if (( parse_result < 0) || (parse_result == 0 && wfd_msg.wfd_attributes == NULL) ) {
			...
			wpa_printf(MSG_INFO, "No Reply PROV-DISC-REQ: WFD enable, Wrong WFD IE or No WFD IE\n");
			return ;
		}
	}
	wfd_parse_free(&wfd_msg);
	...
	if((msg.wps_config_methods & p2p->cfg->config_methods) == 0){ 
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, P2P_EVENT_PROV_DISC_FAILURE 
			"Config Methods in Provision Discovery Request, config=0x%x", p2p->cfg->config_methods); 
		goto out; 
	} 
...
#if 1 // move the auto reconnection from wpa_suppplicant to WDAL
	if (dev)
	...
	if (msg.wps_config_methods & WPS_CONFIG_DISPLAY) {
		...
		if (dev) { 
			dev->flags |= P2P_DEV_PD_PEER_KEYPAD;
			if(p2p->wfd->cfg->enabled) /* WFD only */
				dev->wps_method = WPS_PIN_KEYPAD;
		}
	} else if (msg.wps_config_methods & WPS_CONFIG_KEYPAD) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Peer " MACSTR
			" requested us to write its PIN using keypad",
	...
	else if (msg.wps_config_methods & WPS_CONFIG_PUSHBUTTON) {
		if (dev) {
			if(p2p->wfd->cfg->enabled) /* WFD only */
			   ...
			   int join, int force_freq)
{
	...
	if (p2p_send_action(p2p, freq, dev->info.p2p_device_addr,
			    ...
			    wpabuf_head(req), wpabuf_len(req), 250) < 0) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
			"P2P: Failed to send Action frame");
		wpabuf_free(req);
		return -1;
