...
static void p2p_device_free(struct p2p_data *p2p, struct p2p_device *dev)
{
	...
	wpabuf_free(dev->wfd_info.wfd_subelems);
...
#ifdef CONFIG_WFD
		      struct p2p_device *dev, struct p2p_message *msg, const u8 *data, size_t len)
#else
...
#ifdef CONFIG_WFD
	p2p_add_dev_info(p2p, addr, dev, msg, data, len);
#else
...
struct wpabuf * p2p_build_probe_resp_ies(struct p2p_data *p2p)
{
	...
	if (p2p->num_groups) {
		p2p_buf_add_capability(buf, p2p->go_dev_capab, p2p->go_group_capab);
	} else {
			    ...
			    size_t ie_len)
{
	...
	parse_result = wfd_parse_ies(ie , ie_len, &wfd_msg);
	/* parse_result: 
	 * -1: with WFD IE but incorrect value
	 * 0 and wfd_msg.wfd_attributes is NULL: No WFD IE
	 * 0 and wfd_msg.wfd_attributes is not NULL: with WFD IE and correct content
	 */

	if (p2p->wfd->cfg->enabled ) {
		if (( parse_result < 0) || (parse_result == 0 && wfd_msg.wfd_attributes == NULL) ) {
			wfd_parse_free(&wfd_msg);
			wpa_printf(MSG_DEBUG, "No Reply PROBE-REQ: WFD enable, Wrong WFD IE or No WFD IE\n");
			return ;
		}
	}
	wfd_parse_free(&wfd_msg);
...
int p2p_set_p2p_sec_dev_types(struct p2p_data *p2p, const u8 dev_types[][8],
			  size_t num_dev_types)
{
	if (num_dev_types > P2P_SEC_DEVICE_TYPES)
		num_dev_types = P2P_SEC_DEVICE_TYPES;
	p2p->cfg->p2p_num_sec_dev_types = num_dev_types;
	os_memcpy(p2p->cfg->p2p_sec_dev_type, dev_types, num_dev_types * 8);
	return 0;
}
#endif
...
void p2p_scan_ie(struct p2p_data *p2p, struct wpabuf *ies, const u8 *dev_id)
{
	...
	if (p2p->num_groups) {
		p2p_buf_add_capability(ies, p2p->go_dev_capab, p2p->go_group_capab);
	} else {
...
static void p2p_go_neg_req_cb(struct p2p_data *p2p, int success)
{
	...
	if (dev->dialog_token_miss < 3)
		p2p_nego_reg_timeout = 200000;
	else
		p2p_nego_reg_timeout = 500000;

	p2p_set_timeout(p2p, 0, p2p_nego_reg_timeout);
}
...
static void p2p_go_neg_resp_cb(struct p2p_data *p2p, int success)
{
	...
	p2p_set_state(p2p, P2P_CONNECT);
	//merge Jouni's patch to prevent IOT issue of Galaxy and Nexus with JB image.
	...
	if (dev->wfd_info.wfd_supported) {
		...
		if (dev->wfd_info.wfd_subelems) {
			...
			pos += wpa_snprintf_hex(pos, end - pos,
						wpabuf_head(dev->wfd_info.wfd_subelems),
						wpabuf_len(dev->wfd_info.wfd_subelems));
