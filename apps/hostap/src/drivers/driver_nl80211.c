...
static int wpa_driver_nl80211_put_freq_params(struct nl_msg *msg,
				   struct hostapd_freq_params *freq)
{
...
static int wpa_driver_nl80211_set_channel(struct i802_bss *bss,
			       struct hostapd_freq_params *freq, int set_chan)
{
	...
	wpa_printf(MSG_DEBUG,
		   "nl80211: Set freq %d (ht_enabled=%d, sec_channel_offset=%u)",
		   freq->freq, freq->ht_enabled, freq->sec_channel_offset);
	msg = nlmsg_alloc();
	if (!msg)
		return -1;
	...
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);
	if (wpa_driver_nl80211_put_freq_params(msg, freq) < 0)
		goto nla_put_failure;
				 ...
				 struct wpa_driver_associate_params *params)
{
	...
	if (wpa_driver_nl80211_set_mode(&drv->first_bss, nlmode) ||
	    wpa_driver_nl80211_set_channel(&drv->first_bss, &params->freq, 0)) {
		nl80211_remove_monitor_interface(drv);
		return -1;
	}
...
{
	struct i802_bss *bss = priv;
	return wpa_driver_nl80211_set_channel(bss, freq, 0);
}
...
#elif defined (CONFIG_CUSTOM_WFD) /* CONFIG_CUSTOM_WFD only needs private part */
	memset(&ifr, 0, sizeof(ifr));
	memset(&priv_cmd, 0, sizeof(priv_cmd));
	os_memcpy(buf, cmd, strlen(cmd) + 1);
	os_strncpy(ifr.ifr_name, bss->ifname, IFNAMSIZ);
...
#elif defined (CONFIG_CUSTOM_WFD)
	.driver_cmd = wpa_driver_nl80211_driver_cmd,	
#endif /* ANDROID */
#ifdef CONFIG_WLAN_HB
