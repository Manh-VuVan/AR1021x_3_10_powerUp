				  ...
				  struct hostapd_config *conf)
{
	...
	if (ssid->frequency == 0) {
		...
		conf->channel = (ssid->frequency - 2407) / 5;
		   ...
		   (ssid->frequency >= 5745 && ssid->frequency <= 5845)) {
		...
		if (!no_ht && mode && mode->ht_capab) {
			...
			if (conf->hw_mode == HOSTAPD_MODE_IEEE80211A &&
				...
				conf->secondary_channel =
					...
					conf->channel, 
					wpa_s->global->p2p->cfg->op_reg_class);
			     ...
			     struct wpa_ssid *ssid)
{
	...
	// Use best channel if no frequency presetting
	if (ssid->frequency == 0) {
		if (wpa_s->scan_band == 0 &&
			wpa_s->best_overall_freq > 0) {
			params.freq.freq = wpa_s->best_overall_freq;
			wpa_printf(MSG_DEBUG, "Set AP freq based on best overall "
				"channel %d MHz", params.freq.freq);
		}
		else if ((wpa_s->scan_band == 1) &&
			wpa_s->best_24_freq > 0) {
			params.freq.freq = wpa_s->best_24_freq;
			wpa_printf(MSG_DEBUG, "Set AP freq based on best 2.4 GHz "
				"channel %d MHz", params.freq.freq);
		}
		else if ((wpa_s->scan_band == 2) &&
			wpa_s->best_5_freq > 0) {
			params.freq.freq = wpa_s->best_5_freq;
			wpa_printf(MSG_DEBUG, "Set AP freq based on best 5 GHz "
				"channel %d MHz", params.freq.freq);
		}
		else {
			params.freq.freq = 2412;
			wpa_printf(MSG_DEBUG, "Set AP freq to default "
				"channel %d MHz", params.freq.freq);
		}

		ssid->frequency = params.freq.freq;
	}
