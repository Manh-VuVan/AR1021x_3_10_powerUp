...
struct wpa_global {
	...
	int widi_enable; /* 1: enable; 0: disable */
	int widi_sock_l2sd; /* sokcet ID of l2sd */
	int widi_dev_found; /* 1: found widi source */
	int widi_enable_wps;
	char widi_dev_name[32];
	int widi_dev_type[8];
	u32 widi_vendor_id; /* application must config vendor ID before receiving it's L2SD/L2TA */
...
#define WILDCARD_SSID_SCAN ((struct wpa_ssid *) 1)
	...
	struct wpa_blacklist *p2p_blacklist;
#endif /* CONFIG_P2P */
	...
	u8 wps_onging_ssid[32];
	size_t wps_onging_ssid_len;
	...
	/* To be assigned as mode(0/1:Indoor/Outdoor)+1
	 * 0: swow stop
	 * 1: Indoor mode
	 * 2: Outdoor mode
	 */
	...
	u16 single_channel_scan;
	u16 single_channel_scan_setting;
	/* Band selection for scan
	 * 0: all bands
	 * 1: 2.4GHz band only
	 * 2: 5GHz band only
	 */
	int scan_band;	
	struct os_time p2p_roc_started;
};
