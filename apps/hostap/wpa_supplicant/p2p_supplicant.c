						 ...
						 u8 channel, u8 bw)
{
	...
	printf("wpas_p2p_verify_channel channel=%d bw=%d\n", channel, bw);
...
int wpas_p2p_get_ht40_mode(struct wpa_supplicant *wpa_s,
			   struct hostapd_hw_modes *mode, u8 channel, u8 reg_class)
{
	int op;
	...
	for (op = 0; op_class[op].op_class; op++) {
		...
		printf("op_class=%d reg_class=%d\n", o->op_class, reg_class);

		if(o->op_class != reg_class)
			continue;
