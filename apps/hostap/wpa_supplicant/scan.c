			 ...
			 struct wpa_driver_scan_params *params)
{
	...
	if (wps || wpa_s->global->widi_enable_wps)
#else
	if (wps) 
...
#ifdef CONFIG_CUSTOM_WFD
		wps_ie = wps_build_probe_req_ie(wpa_s->global->widi_enable_wps, wpa_s->global->widi_dev_name, 
						wps == 2 ? DEV_PW_PUSHBUTTON :
						DEV_PW_DEFAULT,
						&wpa_s->wps->dev,
...
#ifdef CONFIG_P2P
	if (strncmp(wpa_s->ifname, "p2p", 3) == 0 && wps) {
		...
		if(scan_req != 2){
			for (tssid = wpa_s->conf->ssid; tssid; tssid = tssid->next) {
				if (wpas_network_disabled(wpa_s, tssid))
					continue;
				if ((params.freqs || !freqs_set) && tssid->scan_freq && tssid->scan_freq_count<5) {
					int_array_concat(&params.freqs,
        						 tssid->scan_freq);
					tssid->scan_freq_count++;
				} else {
...
ssid_list_set:
	...
	if (params.freqs == NULL && wpa_s->single_channel_scan) {
		wpa_dbg(wpa_s, MSG_DEBUG, "set freq %d for scan\n", wpa_s->single_channel_scan);
		params.freqs = os_zalloc(2 * sizeof(int));
		params.freqs[0] = wpa_s->single_channel_scan;
		wpa_s->single_channel_scan = 0;
	}
	...
	params.scan_band = wpa_s->scan_band;
