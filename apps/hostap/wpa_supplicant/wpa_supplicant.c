 ...
 * Returns: The state name as a printable text string
 */
const char * wpa_supplicant_auth_txt(int auth_alg)
{
	switch (auth_alg) {
	...
	case WPA_AUTH_ALG_SHARED:
		return "SHARED";
	case (WPA_AUTH_ALG_OPEN | WPA_AUTH_ALG_SHARED):
		return "OPEN/SHARED";
...
void WIDI_set_state_up(struct wpa_supplicant *wpa_s, int state)
{
	l2_msg_t l2_notif;
	struct sockaddr_in local;

	if(wpa_s->global->widi_enable == 0){
		wpa_dbg(wpa_s, MSG_DEBUG, "%s: WIDI is not initialized", __func__);		
		return;
	}
	...
	memset(&l2_notif, 0, sizeof(l2_notif));

	l2_notif.u.l2sd_assoc_msg.assoc_stat = state;

	if (state == WPS_FAIL_CHECK) {
		l2_notif.u.l2sd_assoc_msg.assoc_stat = L2SDTA_WPS_STATUS_FAIL;
	} else if (state == WPS_DEAUTHENTICATED) {
		l2_notif.u.l2sd_assoc_msg.assoc_stat = L2SDTA_DEAUTHENTICATED;
	} else {
		l2_notif.u.l2sd_assoc_msg.assoc_stat = state;
	}
	...
	if(wpa_s->global->widi_sock_l2sd)
		sendto( wpa_s->global->widi_sock_l2sd, &l2_notif, sizeof(l2_notif), 
		0,(struct sockaddr*)&local, sizeof(local) );
	else
		wpa_dbg(wpa_s, MSG_ERROR, "%s: WIDI socket is not initialized", __func__);

	return;
		
}

void WIDI_set_source_info(struct wpa_supplicant *wpa_s)
{
	l2_msg_t l2_notif;
	struct sockaddr_in local;
	...
	if( strcmp(wpa_s->ifname, "wlan0") ) { /* p2p0 should return here */
		wpa_dbg(wpa_s, MSG_INFO, "%s: not for L2SD interface", __func__);		
		return;
	}
		
	if ((wpa_s->current_ssid == NULL) || (wpa_s->current_ssid->passphrase == NULL)){
		wpa_printf(MSG_INFO, "%s:%d ssid or passphrase is empty", __func__, __LINE__);
		return;
	}
	...
	os_memcpy(l2_notif.u.l2sd_QCA_profile_sync_msg.ssid, wpa_s->current_ssid->ssid, wpa_s->current_ssid->ssid_len);
	strncpy((char *)l2_notif.u.l2sd_QCA_profile_sync_msg.passphrase,
	        wpa_s->current_ssid->passphrase,
	        sizeof(l2_notif.u.l2sd_QCA_profile_sync_msg.passphrase));
	l2_notif.u.l2sd_QCA_profile_sync_msg.passphrase[sizeof(l2_notif.u.l2sd_QCA_profile_sync_msg.passphrase) - 1] = '\0';
	os_memcpy(l2_notif.u.l2sd_QCA_profile_sync_msg.bssid, wpa_s->current_ssid->bssid, ETH_ALEN);	

	l2_notif.msg_type = L2SDTA_MSG_TYPE_WPS_STATUS;
...
static void defer_tx_power_set(void *eloop_ctx, void *timeout_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;
	char cmd[32];
	int len;

	len = os_snprintf(cmd, sizeof(cmd), "%d", wpa_s->conf->tx_power);
	cmd[sizeof(cmd) - 1] = '\0';
	wpa_drv_wlan_debugfs_cmd(wpa_s, "txpower",cmd, len);
}
			      ...
			      enum wpa_states state)
{
	...
	if (state == WPA_COMPLETED){
		WIDI_set_state_up(wpa_s, L2SDTA_ASSOCIATED);
	} else if (state == WPA_DISCONNECTED){
		WIDI_set_state_up(wpa_s, L2SDTA_DISASSOCIATED);
	}
#endif /* CONFIG_CUSTOM_WFD */   
...
#ifdef CONFIG_P2P
		extern wpa_check_p2p_listen_resume(struct wpa_supplicant *wpa_s);
    ...
    if(wpa_s->wpa_state >= WPA_ASSOCIATED) {
        if((wpa_s->conf->tx_power > 0) && (wpa_s->conf->tx_power <= 100)) {
            eloop_cancel_timeout(defer_tx_power_set, wpa_s, NULL);
            eloop_register_timeout(5, 0, defer_tx_power_set, wpa_s, NULL);
        }
    }
}
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
...
#ifdef CONFIG_P2P
	extern wpa_check_p2p_listen_stop(struct wpa_supplicant *wpa_s);
	...
	if (wpa_s->global->p2p) {
		...
		int res;
		int p2p_group;
		p2p_group = (wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_CAPABLE) ? 1 : 0;
				   ...
				   struct wpa_ssid *ssid)
{
	...
	if (ssid == NULL) {
		...
		if (wpa_s->current_ssid == NULL) {
			 ...
			 */
			wpa_s->reassociate = 1;
			if (ssid->mode == WPAS_MODE_AP && ssid->frequency == 0) {
				wpa_bss_flush(wpa_s);
				wpa_s->scan_band = wpa_s->conf->scan_band;
				wpa_msg(wpa_s, MSG_DEBUG, "Set scan band=%d for SAP mode", wpa_s->scan_band);
			}
