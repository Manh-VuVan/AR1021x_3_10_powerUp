...
{
	int idx;
	/* to prevent duplicate sec_device_type */
	u8 i, sec_device_type_check[WPS_DEV_TYPE_LEN]={0};
	...
	/* to prevent duplicate sec_device_type */
	if (wps_dev_type_str2bin(pos, sec_device_type_check))
		return -1;

	for(i = 0; i < idx; i++){
		if (os_memcmp(config->sec_device_type[i], sec_device_type_check, WPS_DEV_TYPE_LEN) == 0)
			return -1;
	}
...
static int wpa_config_process_p2p_sec_device_type(
	const struct global_parse_data *data,
	struct wpa_config *config, int line, const char *pos)
{
	int idx;
        /* to prevent duplicate p2p_sec_device_type */
 	u8 i, sec_device_type_check[WPS_DEV_TYPE_LEN]={0};	

	if (config->p2p_num_sec_device_types >= MAX_SEC_DEVICE_TYPES) {
		wpa_printf(MSG_ERROR, "Line %d: too many p2p_sec_device_type "
			   "items", line);
		return -1;
	}

	idx = config->p2p_num_sec_device_types;
	...
	for(i=0; i<idx; i++){
		if(os_memcmp(config->p2p_sec_device_type[i], sec_device_type_check, WPS_DEV_TYPE_LEN) == 0)
			return -1;
	}	

	if (wps_dev_type_str2bin(pos, config->p2p_sec_device_type[idx]))
		return -1;

	config->p2p_num_sec_device_types++;
	return 0;
}
#endif
...
#ifdef CONFIG_CUSTOM_WFD
	{ FUNC(p2p_sec_device_type), CFG_CHANGED_P2P_SEC_DEVICE_TYPE },
#endif
