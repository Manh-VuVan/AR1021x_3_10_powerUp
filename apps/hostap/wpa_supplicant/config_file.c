...
#define INT_DEFe(t, def) write_int(f, #t, ssid->eap.t, def)
	...
	INT(scan_ssid);
	if(ssid->scan_freq)
		write_int(f, "scan_freq", ssid->scan_freq[0], 0);
	write_bssid(f, ssid);
	write_psk(f, ssid);
	write_proto(f, ssid);
...
static void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
	...
	if (config->p2p_join_timeout)
		fprintf(f, "p2p_join_timeout=%u\n", config->p2p_join_timeout);
#endif /* CONFIG_P2P */
#ifdef CONFIG_WFD
