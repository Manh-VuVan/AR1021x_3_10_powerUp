...
/**
 * @brief Handlers for noise floor-related commands.
 */
static athcfg_cmd_t nf_cmds[] =
{
    {
        .cmd_name = "get",
        .handler_present = TRUE,
...
int 
nf_get(char *ifrn_name, int nargs, char *args[])
{
    athcfg_wcmd_t i_req;
    athcfg_wcmd_nf_t nf;

    memset(&i_req,0, sizeof(athcfg_wcmd_t));
    memset(&nf, 0, sizeof(athcfg_wcmd_nf_t));
	strncpy(i_req.iic_ifname, ifrn_name, strlen(ifrn_name));
	i_req.iic_cmd = ATHCFG_WCMD_GET_NF;

	if (athcfg_do_ioctl(ATHCFG_WCMD_IOCTL, &i_req) < 0)
		return EIO;
    
	nf = i_req.d_nf;
	printf("NF Chan0 master = %d\n",nf.nf_ch0_master);
	printf("NF Chan0 extension = %d\n",nf.nf_ch0_extension);
	printf("NF Chan1 master = %d\n",nf.nf_ch1_master);
	printf("NF Chan1 extension = %d\n",nf.nf_ch1_extension);
    return 0;
}
