...
unsigned char tmpser[64]="123456789";
...
unsigned short reg_domain = 0x8348;
...
//      memcpy(tmpsp,ath_info.serial,sizeof(ath_info.serial));
	...
	if (strncmp(filename, AR6004_HW_1_3_BOARD_DATA_FILE, strlen(AR6004_HW_1_3_BOARD_DATA_FILE)) == 0) {
		...
		*fw = fw_bdata;
		  ...
		  if((ar->product_id==0x1021) || (ar->product_id==0x1022)) {
			printk("AR1022 use dual band board data\n");
		...
		*fw = fw_bdata_6006_1_1;
		   ...
		   (strncmp(filename, tmpfilename, strlen(tmpfilename)) == 0)) {
		printk("Load AR6006/HW1.1 firmware!!!\n");
